...
subtitle: Paper "Cellwise outlier detection in heterogeneous populations" by Zaccaria, G., García-Escudero, L.A., Greselin, F., Mayo-Íscar, A.- Simulation Study - Main Article - Section 3, Figures 1-2 and Table 1, Scenario 1
author: "Zaccaria, G."
date: "April 16, 2025"
output: html_document
---

This file reproduces part of the simulation study presented in Section 3 of the Main Article, specifically the panels in Figures 1 and 2 referring to Scenario 1. Estimated computation time: 40-90 minutes.
...
required_packages <- c("MASS", "pracma", "purrr", "mvnfast", "cellWise", "mclust", "tclust", "doParallel", "parallel", "foreach", "ContaminatedMixt", "mvtnorm", "mnormt", "cluster", "rootSolve", "xtable", "ggplot2", "tidyr", "dplyr", "patchwork")
...
# Load snipEM and MixtureMissing from the .tar.gz files. If the latter is already installed, the code checks for the corresponding version. Version 1.0.2 is required, as it contains the functions "MNM" and "MtM".
if (!require("snipEM", character.only = TRUE)) {
    message("Package snipEM is not installed. Attempting to install now.")
    install.packages("snipEM_1.0.1.tar.gz", repos = NULL, type = "source")
  }
library("snipEM", character.only = TRUE)

if (!require("MixtureMissing", character.only = TRUE)) {
    message("Package MixtureMissing is not installed or a different version from the necessary one is installed. Attempting to install now.")
  install.packages("MixtureMissing_1.0.2.tar.gz", repos = NULL, type = "source")
} else if (require("MixtureMissing", character.only = TRUE) && packageVersion("MixtureMissing") != "1.0.2") {
  remove.packages("MixtureMissing") # Specify the 'lib' if necessary
  install.packages("MixtureMissing_1.0.2.tar.gz", repos = NULL, type = "source")
}
library("MixtureMissing")
```

Start the code profiling.
...
stat.W.tot <- rbind(stat.W.tot, stat.W)
```
...
OUTLIER DETECTION [Table 1 for Scenario 1 - Main Article]
